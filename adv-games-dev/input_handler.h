...
public:
		...
		else
		{
			if (SDL_NumJoysticks() >= 1)
			{
				...
				else
				{
					...
					{
						if (SDL_HapticRumblePlay(_self->gControllerHaptic_01, 0.75, 1500) != 0)
						{
    ...
    int HandleInputEvent(SDL_Event &e, engine eng)
    {
		...
		case SDL_JOYDEVICEADDED:
		{
			if (_self->JOYSTICK_ACCUM < _self->JOYSTICK_INIT_COUNT)
			{
				...
				if (_self->JOYSTICK_ACCUM == _self->JOYSTICK_INIT_COUNT) _self->JOYSTICK_INIT_FLAG = true;
				return 0;
			}
			else
			{
        ...
        eng.change_state(std::to_string(state), engine::get());
    }
